...
ComponentManager::ComponentManager(Uint32 maxSize)
	:m_positionPool(maxSize, sizeof(TransformComponent))
...
ComponentID ComponentManager::allocateTransformComponent(const TransformData& data, bool shouldWrap)
{
	...
	if (ptr != NULL)
	{
		...
		TransformComponent* pComponent = new (ptr)TransformComponent(newID,shouldWrap);
...
void ComponentManager::updatePhysics(float elapsedTime)
{
	for (auto& it : m_physicsCOMPONENT_MANAGER)
	{
		...
		TransformComponent* pPosition = getTransformComponent(pPhysics->m_positionID);
...
void ComponentManager::updateSteering(float elapsedTime)
{
	for (auto& it : m_steeringCOMPONENT_MANAGER)
	{
		...
		PhysicsComponent* pPhysics = getPhysicsComponent(pSteering->m_physicsID);
		assert(pPhysics != NULL);
