...
public:
	...
	struct MouseState
	{
		...
		{
			state.mStateNew[Buttons::Left] = glfwGetMouseButton(GRAPHICS->m_window, GLFW_MOUSE_BUTTON_LEFT) == 1;
			state.mStateNew[Buttons::Right] = glfwGetMouseButton(GRAPHICS->m_window, GLFW_MOUSE_BUTTON_RIGHT) == 1;
			state.mStateNew[Buttons::Middle] = glfwGetMouseButton(GRAPHICS->m_window, GLFW_MOUSE_BUTTON_MIDDLE) == 1;
		}
