...
void PhysicsComponent::integrate(PositionComponent& posComponent, float elapsedTime)
{
	...
	{
		//cap accelerations
		if (glm::length(mData.acc) * glm::length(mData.acc) > (mData.maxAccMagnitude * mData.maxAccMagnitude))
		{
		...
		//cap velocities
		if (glm::length(mData.vel) * glm::length(mData.vel)  > (mData.maxSpeed * mData.maxSpeed) )
		{
